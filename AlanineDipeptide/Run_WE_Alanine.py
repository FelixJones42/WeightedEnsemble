...
if __name__ == '__main__':
    ...
    for k in range(n_we_runs):
        ...
        for j in range(n_we_iters):
            ...
            if j>0:
                if np.dot(B.ν,v)>0:
                    ...
                    failed_to_resample = failed_to_resample + 1
        ...
        print('Run ',k+1,' of ',n_we_runs,' complete.',' estimate = ',weight_flux, 'num_target_hits = ',num_target_hits,'failed to resample ',failed_to_resample, ' times',flush = True)
    ...
    WEestimatefile = open('Est_WE_yes_reinjection.data','wb')
    pickle.dump(estimate, WEestimatefile)
    WEestimatefile.close()
